...
function createWindowFixture(pdf = Buffer.from("%PDF-1.7\nexample")) {
  ...
  const BrowserWindow = vi.fn(function () {
    return { loadURL, webContents: { executeJavaScript, printToPDF }, destroy };
  });
  return { BrowserWindow, loadURL, executeJavaScript, printToPDF, destroy };
}
...
describe("PDF export service", () => {
  test("prints an A4 document and destroys the hidden window", async () => {
    ...
    expect(fixture.executeJavaScript).toHaveBeenNthCalledWith(1, expect.stringContaining("document.write(\"<!doctype html>"));
    expect(fixture.executeJavaScript).toHaveBeenNthCalledWith(2, "document.fonts ? document.fonts.ready : Promise.resolve()");
  ...
  test("builds printable HTML without adding visible metadata", () => {
    ...
    expect(html).toContain(".ostracon-pdf-document .mermaid svg");
  ...
  test("uses the latest print settings for each export", async () => {
    ...
    expect(fixture.executeJavaScript).toHaveBeenCalledWith(expect.stringContaining("max-height: 8000px !important"));
  });
});
